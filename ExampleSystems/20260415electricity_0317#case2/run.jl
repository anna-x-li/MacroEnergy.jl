...
# =========================
# 0) 生成 log 文件（带时间戳，避免覆盖）
# =========================
timestamp = Dates.format(now(), "yyyymmdd_HHMMSS")
log_file = joinpath(@__DIR__, "macro_run_log_$timestamp.txt")

# =========================
# 1) Windows：读取进程峰值 Working Set（RSS）——无需采样，几乎零开销
# =========================
if Sys.iswindows()
...
function peak_working_set_bytes()
    if Sys.iswindows()
        ...
        # 非 Windows：尽量用 Sys.maxrss()（不保证所有平台/版本行为一致）
        try
            return Sys.maxrss()
        catch
            return missing
