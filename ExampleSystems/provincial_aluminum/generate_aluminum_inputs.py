...
def compute_scrap_max_supply_by_province(
    ...
    """Compute per-timestep scrap max_supply (t/hr) per province using weights.

    max_supply is a per-timestep constraint, so we convert annual Mton/year to
    an hourly rate (t/hr = Mton/year * 1e6 / 8760).
    ...
    total_scrap_ton_per_hr = float(scrap_mid_mton_per_year) * 1_000_000.0 / 8760.0
    ...
    for prov in PROVINCE_TO_REGION.keys():
        ...
        caps[prov] = total_scrap_ton_per_hr * share
    return caps
    ...
    with open(out_path, "w", encoding="utf-8", newline="") as f:
        ...
        w.writerow(
            [
                ...
                "WeightShare",
                "MaxSupply_ton_per_hr",   # per-timestep limit used as max_supply in model
                "MaxSupply_288h_tons",    # reference only: total over 288 timesteps
            ]
        )
        for period, year in PERIOD_YEAR_MAP.items():
            ...
            for prov, (region_str, _region_num) in PROVINCE_TO_REGION.items():
                ...
                cap_ton_per_hr = float(caps_by_province.get(prov, 0.0))
                cap_288h = cap_ton_per_hr * 288.0
                w.writerow(
                    [
                        prov,
                        region_str,
            ...
            total_ton_per_hr = scrap_mid_mton_per_year * 1_000_000.0 / 8760.0
            w.writerow(
                [
                    "TOTAL",
                    "",
