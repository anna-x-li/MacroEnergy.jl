...
3. **AluminumScrap nodes** (resource, one per province with capacity)
   - `type`: "AluminumScrap"
   - `time_interval`: "AluminumScrap"
   - `constraints`: {"BalanceConstraint": true}
   - `instance_data`: one entry per province with capacity
     - `id`: "aluminumscrap_source_Region{number}{province_name}"
     - `max_supply`: [<from data or default>]
     - `price_supply`: [0]

4. **Bauxite nodes** (resource, one per province with capacity)
   - `type`: "Bauxite"
...
5. **Graphite nodes** (resource, one per province with capacity)
   - `type`: "Graphite"
